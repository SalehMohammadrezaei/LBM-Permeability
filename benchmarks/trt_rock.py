...
p.add_argument('--crop', type=int, nargs=2, default=[58, 442]); p.add_argument('--output', required=True)
...
lo, hi = a.crop
blocked = np.ascontiguousarray(image[lo:hi, lo:hi, lo:hi] == a.solid_value)
...
for case in json.loads(a.cases):
    ...
    r = lbm_stokes_3d(blocked, F_x=force[0], F_y=force[1], F_z=force[2], tau=case['tau'],
                      collision=case['collision'], backend=case['backend'], verbose=False,
                      n_steps_max=case.get('steps', 150000), conv_tol=a.tol, conv_window=100, consecutive=3,
                      characteristic_length=a.length, wall_timeout_s=case.get('timeout', 5400), return_fields=False)
    r.update(case=case, dataset=str(a.dataset), dataset_sha256=hashlib.sha256(raw).hexdigest(), crop=a.crop,
